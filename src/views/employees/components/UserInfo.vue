<template>
  <div class="user-info">
    <!-- 个人信息 -->
    <el-form v-loading="loading" label-width="220px">
      <!-- 工号 入职时间 -->
    ...
    <el-form v-loading="loading" label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <UploadImg ref="staffImgs" :limit="4" />
        </el-form-item>
...
import { getUserBaseInfo, saveUserBaseInfo, getUserPersonInfo, saveUserPersonInfo } from '@/api/employees'
export default {
  data() {
    return {
  ...
  methods: {
    async getUserBaseInfo() {
      ...
      console.log(this.$refs.staffImg)
      // 将值赋给图片上传组件中的fileList
      if (this.userInfo.staffPhoto) {
        this.$refs.staffImg.fileList = [{ url: this.userInfo.staffPhoto, upload: true }]
      }
      this.loading = false
    },
    async saveUser() {
      const imgfile = this.$refs.staffImg.fileList
      console.log(imgfile)
      if (!imgfile[0].upload) {
        this.$message.warning('图片还没上传完成')
        return false
      }
      this.userInfo.staffPhoto = imgfile[0].url
      await saveUserBaseInfo({ ...this.userInfo, id: this.userId })
      this.$message.success('基本信息更新成功')
    },
    async getUserPersonInfo() {
      this.loading = true
      this.formData = await getUserPersonInfo(this.userId)
      // 如果获取的数据中有图片，将图片转化为数组存入上传图片组件中的fileList中
      if (this.formData.staffPhoto) {
        this.$refs.staffImgs.fileList = this.formData.staffPhoto.split(',').map(item => ({ url: item }))
      }
      this.loading = false
    },
    async savePersonal() {
      const imgsfile = this.$refs.staffImgs.fileList
      if (!imgsfile.some(item => item.upload)) {
        this.$message.warning('图片还没上传完成')
        return false
      }
      console.log(imgsfile)
      this.formData.staffPhoto = imgsfile.map(item => item.url).join(',')
      console.log(this.formData.staffPhoto)
      await saveUserPersonInfo({ ...this.formData, id: this.userId })
