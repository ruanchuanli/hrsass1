...
import { getUserJobInfo, saveUserJobInfo, getEmployeesSimple } from '@/api/employees'
export default {
  data() {
    return {
  ...
  methods: {
    ...
    async getUserJobInfo() {
      this.loading = true
      this.formData = await getUserJobInfo(this.userId)
      this.loading = false
    },
    // 更新岗位信息
    async saveJob() {
      this.loading = true
      await saveUserJobInfo({ id: this.userId, ...this.formData })
