      ...
      <el-card>
        ...
        <div id="printinfo">
          <el-row type="flex" justify="end" style="margin-top: 20px">
            <el-button v-print="userprint" type="primary" size="mini">打印</el-button>
          </el-row>
          <div v-if="type === 'personal'">
            ...
            <table cellspacing="0" width="100%" class="tableList">
              ...
              <tr>
                ...
                <td rowspan="5" style="width:10%"><img style="width:155px;height:218px" :src="userInfo.staffPhoto"></td>
              ...
              <tr>
                <th>出生日期</th>
                <td colspan="6">{{ formData.dateOfBirth? (formData.dateOfBirth | formatDate):'' }}</td>
              </tr>
              <tr>
                <th>最高学历</th>
              ...
              <tr>
                <th>员工照片</th>
                <td><img v-for="(item, index) in formData.staffPhoto" :key="index" :src="item" alt="" style="width:100px;height:100px;margin-right:5px"></td>
                <th>生日</th>
...
<script>
import { getUserPersonInfo, getUserJobInfo } from '@/api/employees'
...
export default {
  ...
  // 创建完毕状态
  created() {
    this.type === 'personal' ? this.getUserPersonInfo() : this.getUserJobInfo()
  },
  // 组件更新
  methods: {
    async getUserPersonInfo() {
      this.formData = await getUserPersonInfo(this.userId) // 获取个人基本信息
      this.userInfo = await getUserDetailinfo(this.userId)
      this.formData.staffPhoto = this.formData.staffPhoto.split(',')
    },
    async getUserJobInfo() {
      const userInfo = await getUserDetailinfo(this.userId)
      const jobInfo = await getUserJobInfo(this.userId) // 获取个人基本信息
